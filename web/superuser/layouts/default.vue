<template>
  <div>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark" style="background-color:  #535C69">
      <a class="navbar-brand">АРМ суперпользователя</a>
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        ...
        <div class="navbar-nav justify-content-end ml-auto">
          ...
          <a class="nav-item nav-link" :class="$nuxt.$route.path == '/' ?  'disabled' : 'active'" href=""
            title="Полный список анкет клиентов">
            <i class="fas fa-list"></i>&nbsp;&nbsp;Все анкеты
          </a>
          
          <a  class="nav-item nav-link" :class="$nuxt.$route.path == '/defected/' ?  'disabled' : 'active'" href="defected"
            title="Забракованные анкеты">
            <i class="fas fa-search-minus"></i>&nbsp;&nbsp;Забракованные
          </a>
<!--
          <a  class="nav-item nav-link" :class="$nuxt.$route.path == '/passport-report/' ?  'disabled' : 'active'" href="passport-report"
            title="Отчёт по обработанным паспортистами анкетам">
            <i class="fas fa-list-alt"></i>&nbsp;&nbsp;Отчёт по паспортистам
          </a>
          -->
...
export default Vue.extend({
  ...
  methods: {        
    ...
    //unload tab event listener
    /*async onBeforeUnloadListener(event: any) {
      // Cancel the event as stated by the standard.
      event.preventDefault();
      console.log(event)
      // Older browsers supported custom message
      event.returnValue = 'Охуел?';
    },*/
  },

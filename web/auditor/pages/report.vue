    ...
    <div>    
      ...
      <Message v-if="errorMessageVisible == true" :severity="this.$store.state.fetchError.fetchErrorLevel==0 ? 'error' : 'warn'">{{this.$store.state.fetchError.fetchErrorMessage}}</Message>
        ...
        <DataTable :value="this.list" class="p-datatable-striped p-datatable-gridline" autoLayout>    
            <template #empty>
                За указанный период не введено ни одной анкеты
            </template>        
            <Column field="name" header="Оператор"></Column>
            <Column header="Всего анкет / клиентов">
                <template #body="slotProps">
                    {{slotProps.data.total_forms}} / {{slotProps.data.total_customers}}
                </template>
            </Column>
            <Column header="Всего анкет / клиентов Москва и МО">
                <template #body="slotProps">
                    {{slotProps.data.total_moscow_forms}} / {{slotProps.data.total_moscow_customers}}
                </template>
            </Column>
            <Column header="Всего анкет / клиентов регионы">
                <template #body="slotProps">
                    {{slotProps.data.total_not_moscow_forms}} / {{slotProps.data.total_not_moscow_customers}}
                </template>
            </Column>
            <Column header="Верифицировано аудитором анкет / клиентов">
                <template #body="slotProps">
                    {{slotProps.data.verified_forms}} / {{slotProps.data.verified_customers}}
                </template>
            </Column>
             <Column header="Верифицировано аудитором анкет / клиентов Москва и МО">
                <template #body="slotProps">
                    {{slotProps.data.verified_moscow_forms}} / {{slotProps.data.verified_moscow_customers}}
                </template>
            </Column>
             <Column header="Верифицировано аудитором анкет / клиентов регионы">
                <template #body="slotProps">
                    {{slotProps.data.verified_not_moscow_forms}} / {{slotProps.data.verified_not_moscow_customers}}
                </template>
            </Column>
            <Column field="defected_customers" header="Забраковано аудитором анкет / клиентов">
                <template #body="slotProps">
                    {{slotProps.data.defected_forms}} / {{slotProps.data.defected_customers}}
                </template>
            </Column>
        </DataTable>  
    </div>
...
<script lang="ts">
    ...
    export default Vue.extend({
        ...
        head() {
            return {
                //заголовок браузера
                title: 'Отчёт по операторам | АРМ аудитора ИС Верификатор',           
            }
        },
        async beforeCreate() {             
            this.$store.commit('initializeStore');
            this.$store.commit('CHANGE_TITLE','Отчёт по операторам');           
        }, 
        //-----------------------------------------------------------
        //загрузка страницы    
        async mounted() {
        ...
        methods: {
            ...
            async makeReport() {
               // alert(moment((<any>this.date_from).trim(),"DD.MM.YYYY HH:mm").isValid())
                ...
                try {
                    const res = await this.$nuxt.$http.$post('/api/auditor/report',searchParams);

...
export const getters: GetterTree<RootState, RootState> = {
  ...
  currentImportId: state => state.currentImportId,
...
export const mutations: MutationTree<RootState> = {
  initializeStore: (state) => {
      ...
      const currentImportId = window.sessionStorage.getItem('currentImportId');      
      if(currentImportId) {        
          state.currentImportId = currentImportId;       
      }  else {
        window.sessionStorage.setItem('currentImportId','0')
      }
  },
  ...
  CHANGE_CURRENTIMPORTID: (state, newCurrentImportId: string) => {
    state.currentImportId = newCurrentImportId;
      window.sessionStorage.setItem('currentImportId',state.currentImportId);
  }, 
